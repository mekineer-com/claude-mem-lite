...
function initTestDb(tmpHome) {
  ...
  db.exec(`
    ...
    CREATE UNIQUE INDEX idx_sess_memory_sid ON sdk_sessions(memory_session_id);
    CREATE INDEX idx_obs_epoch_project ON observations(created_at_epoch DESC, project);
...
function runHook(event, { stdin, env = {}, args = [] } = {}) {
  const mergedEnv = {
    ...
    CLAUDE_PROJECT_DIR: env.CLAUDE_PROJECT_DIR || PROJECT_DIR,
