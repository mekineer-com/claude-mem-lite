...
A six-round real-user E2E audit (CLI surface → MCP server → hook execution paths → injection/recall) produced four user-facing correctness fixes plus the first A/B harness that measures the *UserPromptSubmit injection* path directly. This release also ships previously-unreleased work that had accumulated on main: the task-imperative Phase-2 emitter (default OFF) and the cite-recall / denoise-ab benchmark instrumentation. Suite 3367 → 3403 (+36; +12 from the audit), ESLint clean.

**fix (display): search/recent no longer mis-tag a current, top-importance memory as [SUPERSEDED].** `markSuperseded` picked the "newest" obs per file by sorting on `r.date`, but the production search pipeline supplies `created_at`/`created_at_epoch` and no `date` — so the sort was a silent no-op and supersession degraded to relevance order, flagging genuinely-current rows. Now sorts on `created_at_epoch ?? created_at ?? date`. Display-only tag (no recall/ranking impact); unit tests passed because their fixtures supplied `date` — a test/production field-shape drift.
