...
function importToolPair(db, toolUse, toolResult, project, seenObs) {
  const sessionId = toolUse.sessionId || 'imported';
  const useId = toolUse.tool_use_id || toolUse.id;
  const key = dedupKey([sessionId, useId || '']);
...
export async function importJsonl(db, path, { project }) {
  ...
  const tryImportToolPair = (useEv, resultEv) => {
    ...
    return importToolPair(db, useEv, resultEv, project, seenObs);
  };
