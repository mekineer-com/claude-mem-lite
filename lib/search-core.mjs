 ...
 *   - MIN_FUSION_POOL floor (= stock v6 limit 20 × the 3× buffer): every limit ≤ 20
...
export async function coreRunSearchPipeline(ctx, opts) {
  ...
  // Keep stock v6's limit-20 concept/PRF gates and fetch size for every smaller page.
  // Otherwise limit=5 and limit=10 can expand different candidate sets before slicing.
  const rankingLimit = Math.max(limit, MIN_FUSION_POOL / 3);
  ...
  if (obsTypeFallback && results.length === 0 && obsType) {
    ...
    typeParams.push(perSourceLimit);
    const typeRows = db
      .prepare(
        `
