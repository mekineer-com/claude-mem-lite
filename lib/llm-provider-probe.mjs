...
import { httpConnectProxyFor } from './proxy-fetch.mjs';
 ...
 * then the CLI.
 *
 * @param {{_probe?: Function}} [seams]
 * @returns {Promise<{mode: string, level: 'ok'|'warn', message: string}>}
 */
export async function llmProviderStatus({ _probe = tcpReachable } = {}) {
  ...
  try {
    result = proxy
      ? await _probe(target.hostname, { port: Number(target.port) || 80, proxy, host })
      : await _probe(host, { port: 443, proxy: null });
  } catch (e) {
