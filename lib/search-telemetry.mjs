...
function snapshotLabel(row) {
  const label = row.title || row.subtitle || row.request || row.completed || row.text || row.prompt_text || '(untitled)';
...
export function recordSearch(db, {
  ...
  return withNonblockingWrite(db, () => db.transaction(() => {
    ...
    const run = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      project, scrubSecrets(String(query)).trim().slice(0, 500), surface, searchMode,
...
export function updateSearchCorpusCounts(db, searchId, corpusCounts) {
  return withNonblockingWrite(db, () => db.prepare(
    'UPDATE search_runs SET corpus_counts_json = ? WHERE search_id = ?'
...
export function countHookEligibleCorpus(db, project, cutoff) {
  return {
    obs: count(db, `SELECT COUNT(*) AS c FROM observations WHERE project = ? AND importance >= 1 AND created_at_epoch > ? AND ${liveObsFilterSql('')} AND ${notLowSignalTitleClause('')}`, [project, cutoff]),
...
export function formatSearchTelemetryReport(report) {
  ...
  for (const [surfaceRank, row] of Object.entries(report.by_rank)) {
    ...
    lines.push(surfaceRated >= 30 && surfaceRow.coverage >= 0.2
