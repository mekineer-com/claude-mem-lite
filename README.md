...
- **Automatic capture** -- Hooks into Claude Code lifecycle (PostToolUse, SessionStart, Stop) to record observations without manual effort
- **FTS5 search** -- BM25-ranked full-text search across observations, session summaries, and user prompts
...
| Tool | Description |
|------|-------------|
| `mem_search` | FTS5 full-text search with BM25 ranking. Filters by type, project, date range. |
| `mem_timeline` | Browse observations chronologically around an anchor point. |
| `mem_get` | Retrieve full details for specific observation IDs. |
...
**user_prompts** -- Logged user inputs
```
...
PostToolUse (every tool execution)
  -> Filter noise (skip Read, Glob, screenshots, etc.)
...
Stop
  ...
  -> Spawn LLM summary worker (20s delayed)
```
...
Each observation is categorized with type, title, narrative, concepts, and facts.
