...
export async function rerankEval(entry, { turns = 'user', topK = 20, ks = [1, 5, 10], llm = defaultLlm } = {}) {
  ...
  const cand = baseRows.slice(0, topK).map((r) => ({ sid: idToSession.get(r.id), text: idToText.get(r.id) || '' }));
...
export async function runRerankEval(entries, { turns = 'user', topK = 20, ks = [1, 5, 10], llm = defaultLlm, concurrency = 4, onProgress, onResult } = {}) {
  ...
  const mean = (rows, arm, k) => (rows.length ? rows.reduce((s, r) => s + r[arm][String(k)], 0) / rows.length : 0);
  const agg = (rows) => ({
    n: rows.length,
  ...
  return { config: { turns, topK, ks, concurrency }, n: per.length, parseRate, overall: agg(per), perType, perQuestion: per };
}

// ─── CLI ─────────────────────────────────────────────────────────────────────
function parseArgs(args) {
  const o = { turns: 'user', topK: 20, ks: [1, 5, 10], type: null, max: Infinity, concurrency: 4, out: null, dataset: null };
...
async function main(argv) {
  ...
  if (!o.dataset) {
    process.stderr.write('Usage: node benchmark/longmemeval-rerank.mjs <dataset.json> [--type T] [--max N] [--topK 20] [--concurrency 4] [--out f.jsonl]\n');
    process.exit(1);
  }
  ...
  const row = (label, s) => `  ${label.padEnd(10)} ${o.ks.map((k) => `@${k}=${pct(s[String(k)])}`).join('  ')}`;
