...
function buildFixIndex() {
  ...
  const out = execSync(
    "git -C '" + repo + "' log --extended-regexp --grep='^fix' --format='%H %ct'",
...
function fixInWindow(fixIdx, file, t, w) {
  ...
  if (!arr) return false;
  const lo = t + GAP_MS, hi = t + w;
  for (const e of arr) { if (e > lo && e <= hi) return true; if (e > hi) break; }
  return false;
}
...
function collectEditEvents() {
  ...
  let sessionsSeen = 0, injCount = 0, editTotal = 0;
...
function rate(events, fixIdx, w, pred) {
  let n = 0, s = 0;
  for (const ev of events) { if (!pred(ev)) continue; n++; if (fixInWindow(fixIdx, ev.file, ev.ts, w)) s++; }
...
for (const f of neverInjFiles) {
  const ts = edits.filter((e) => e.file === f).map((e) => e.ts).sort((a, b) => a - b);
  medianTs.set(f, ts[Math.floor(ts.length / 2)]);
}

const fits = (e, w) => e.ts + w <= dataEnd; // censored-safe predicate

const result = { meta: { dir: DIR, sessionsSeen, injCount, editTotal, gapDays: GAP_MS / DAY, dataEnd }, windows: {} };
...
for (const w of WINDOWS) {
  ...
  const before = rate(edits, fixIdx, w, (e) => injectedFiles.has(e.file) && e.ts < firstInj.get(e.file) && fits(e, w));
  const after  = rate(edits, fixIdx, w, (e) => injectedFiles.has(e.file) && e.ts >= firstInj.get(e.file) && fits(e, w));
  ...
  const plBefore = rate(edits, fixIdx, w, (e) => neverInjFiles.has(e.file) && e.ts < medianTs.get(e.file) && fits(e, w));
  const plAfter  = rate(edits, fixIdx, w, (e) => neverInjFiles.has(e.file) && e.ts >= medianTs.get(e.file) && fits(e, w));
  result.windows[w / DAY] = { treated, untreated, withinFile: { before, after }, placebo: { before: plBefore, after: plAfter } };
}
...
console.log(`dir: ${DIR}`);
console.log(`sessions=${sessionsSeen}  file-keyed injections=${injCount}  edit/write events=${editTotal}  gap=${GAP_MS / DAY}d\n`);

const didDeltas = []; // difference-in-differences per window (memory-specific drop)
for (const w of WINDOWS) {
  const r = result.windows[w / DAY];
  const wfDrop = (r.withinFile.before.p != null && r.withinFile.after.p != null) ? r.withinFile.before.p - r.withinFile.after.p : null;
  const plDrop = (r.placebo.before.p != null && r.placebo.after.p != null) ? r.placebo.before.p - r.placebo.after.p : null;
  ...
  console.log(`── window ${w / DAY}d — outcome = file gets a fix: commit in (t+${GAP_MS / DAY}d, t+${w / DAY}d], censored-safe ──`);
  console.log(`  cross-edit (CONFOUNDED by hotness, ignore for verdict):`);
  console.log(`     treated   ${pct(r.treated.p)} ${ci(r.treated.ci)} (n=${r.treated.n})   untreated ${pct(r.untreated.p)} ${ci(r.untreated.ci)} (n=${r.untreated.n})`);
  console.log(`  within-file (hotness-controlled): before ${pct(r.withinFile.before.p)} (n=${r.withinFile.before.n}) → after ${pct(r.withinFile.after.p)} (n=${r.withinFile.after.n})  drop=${wfDrop == null ? 'n/a' : pct(wfDrop)}`);
  console.log(`  placebo  (never-injected, maturation): before ${pct(r.placebo.before.p)} (n=${r.placebo.before.n}) → after ${pct(r.placebo.after.p)} (n=${r.placebo.after.n})  drop=${plDrop == null ? 'n/a' : pct(plDrop)}`);
  if (enoughWf && enoughPl && did != null) {
    didDeltas.push(did);
    console.log(`  → DiD (memory-specific drop = within-file drop − placebo drop) = ${pct(did)}  ${did > 0.03 ? '[+ memory hint]' : did < -0.03 ? '[− against]' : '[≈ explained by maturation]'}`);
  } else {
    console.log(`  → DiD: N too small in within-file and/or placebo arm (need ≥10 each) — no read at this window`);
  }
  console.log('');
}

console.log('VERDICT (reads ONLY the difference-in-differences; cross-edit & raw within-file are confounded):');
if (didDeltas.length === 0) {
  console.log('  INCONCLUSIVE — after censoring + maturation control, sample is too thin for a DiD read.');
  console.log('  The observational gate cannot resolve this — by design it only ever offered a hint. Decide the');
  console.log('  pilot on its own merits: it is the instrument built precisely because observation is this confounded.');
} else {
  const pos = didDeltas.filter((d) => d > 0.03).length;
  const neg = didDeltas.filter((d) => d < -0.03).length;
  const all = didDeltas.length;
  if (pos === all) {
    console.log('  HINT +  EVERY valid window shows injected files dropping post-edit fix-rate MORE than maturation');
    console.log('          predicts (positive DiD throughout). Weak/confounded/observational, but does not contradict');
    console.log('          the efficacy hypothesis. → Pilot justified.');
  } else if (neg === all) {
    console.log('  HINT −  Every valid window: within-file drop fully explained by maturation. No observational support.');
  } else {
    console.log('  WEAK/MIXED  DiD inconsistent across windows (some +, some ≈0). At least one positive window also');
    console.log('          carries a base-rate-asymmetry artifact (placebo = cold files, no room to drop). Net: the');
    console.log('          observational gate is essentially SILENT — it neither supports nor refutes efficacy.');
  }
  console.log('  STRUCTURAL LIMIT: in a fully-dogfooded system, injection is applied to ~every high-risk (hot) file,');
  console.log('  so no comparable UNEXPOSED hot-file control exists. Treatment is confounded with the risk factor by');
  console.log('  construction → observation cannot identify the effect. This is the textbook reason an RCT is required,');
  console.log('  not a defect of this script. Sign-of-DiD only; N small; calendar-time leaks into both arms.');
}
