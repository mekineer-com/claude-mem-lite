...
describe('scanDirectory (flat)', () => {
  ...
  it('discovers .md files directly in directory', () => {
    ...
    writeFileSync(join(agentsDir, 'code-review.md'), '# Code Review Agent\nReviews code for quality and best practices.');
  ...
  it('skips hidden dirs and node_modules', () => {
    ...
    writeFileSync(join(dir, 'good-skill', 'SKILL.md'), '# Good Skill\nWith enough content to pass minimum check.');
...
describe('scanPluginResources', () => {
  it('discovers nested agents/*.md', () => {
    ...
    writeFileSync(join(plugin, 'agents', 'code-expert.md'),
  ...
  it('discovers nested skills/*/SKILL.md', () => {
    ...
    writeFileSync(join(plugin, 'skills', 'my-skill', 'SKILL.md'),
  ...
  it('discovers both agents and skills from a single plugin', () => {
    ...
    writeFileSync(join(plugin, 'agents', 'expert.md'),
      '# Expert Agent\nHandles expert-level tasks with precision.');
    mkDir('plugin-c', 'skills', 'design-patterns');
    writeFileSync(join(plugin, 'skills', 'design-patterns', 'SKILL.md'),
  ...
  it('skips non-.md files in agents/', () => {
    ...
    writeFileSync(join(plugin, 'agents', 'real.md'),
...
describe('scanPluginsDirectory', () => {
  it('scans multiple plugins', () => {
    ...
    writeFileSync(join(agentsDir, 'ui-design', 'agents', 'designer.md'),
      '# Designer\nA UI designer agent for creating interfaces.');
    mkDir('agents', 'ui-design', 'skills', 'responsive');
    writeFileSync(join(agentsDir, 'ui-design', 'skills', 'responsive', 'SKILL.md'),
      '---\nname: responsive\n---\n# Responsive Design');

    // Plugin 2
    mkDir('agents', 'security', 'agents');
    writeFileSync(join(agentsDir, 'security', 'agents', 'scanner.md'),
  ...
  it('skips hidden dirs and non-directories', () => {
    ...
    writeFileSync(join(agentsDir, '.hidden', 'agents', 'x.md'),
      '# Hidden agent with content enough');
    writeFileSync(join(agentsDir, 'some-file.txt'), 'not a dir');
    mkDir('agents2', 'real-plugin', 'agents');
    writeFileSync(join(agentsDir, 'real-plugin', 'agents', 'a.md'),
      '# Real Agent\nWith valid content.');
...
describe('scanAllResources', () => {
  it('finds resources from both flat skills and plugin agents', () => {
    ...
    writeFileSync(join(dataDir, 'managed', 'skills', 'standalone-skill', 'SKILL.md'),
      '---\nname: standalone-skill\n---\n# Standalone Skill\nThis is a standalone skill.');

    mkDir('data', 'managed', 'agents', 'my-plugin', 'agents');
    writeFileSync(join(dataDir, 'managed', 'agents', 'my-plugin', 'agents', 'expert.md'),
      '# Expert\nA domain expert for helping.');
    mkDir('data', 'managed', 'agents', 'my-plugin', 'skills', 'helper');
    writeFileSync(join(dataDir, 'managed', 'agents', 'my-plugin', 'skills', 'helper', 'SKILL.md'),
      '---\nname: helper\n---\n# Helper Skill\nHelps with things.');
  ...
  it('deduplicates by type:name (first wins)', () => {
    ...
    writeFileSync(join(dataDir, 'managed', 'skills', 'shared', 'SKILL.md'),
    ...
    writeFileSync(join(dataDir, 'managed', 'agents', 'plug', 'skills', 'shared', 'SKILL.md'),
      '---\nname: shared\n---\n# Shared from plugin\nPlugin version of the skill.');

    const res = scanAllResources({ dataDir });
    const skillNames = res.filter(r => r.type === 'skill').map(r => r.name).sort();
...
describe('diffResources', () => {
  it('identifies new resources from plugin scan', () => {
    ...
    const scanned = [
      { name: 'my-plugin/expert', type: 'agent', source: 'preinstalled',
        localPath: '/tmp/x', content: 'test', fileHash: 'abc123', repoUrl: null },
      { name: 'my-plugin/helper', type: 'skill', source: 'preinstalled',
        localPath: '/tmp/y', content: 'test2', fileHash: 'def456', repoUrl: null },
    ];
  ...
  it('detects changed hash for plugin resources', () => {
    ...
    const scanned = [
      { name: 'plug/agent-x', type: 'agent', source: 'preinstalled',
        localPath: '/tmp/a', content: 'updated', fileHash: 'newhash', repoUrl: null },
    ];
