...
import {
  REPO, setPhase, check, summary, node, run, makeFakeClaudeBin, sandboxEnv,
  mcpSession, runHook, join, existsSync, readFileSync, writeFileSync, mkdirSync, rmSync,
} from './lib.mjs';
...
const packOut = run('npm', ['pack', '--pack-destination', SBX], { cwd: REPO, env: process.env, timeout: 300_000 });
const tarball = join(SBX, packOut.stdout.trim().split('\n').pop());
check('npm pack produced a tarball', () => ({ ok: existsSync(tarball), detail: `${tarball} ${packOut.stderr.slice(-200)}` }));

const gi = run('npm', ['install', '-g', tarball, '--no-audit', '--no-fund'], { env: ENV, cwd: SBX, timeout: 600_000 });
check('npm i -g succeeds', () => ({ ok: gi.code === 0, detail: `exit=${gi.code} ${(gi.stdout + gi.stderr).slice(-500)}` }));
...
check('the globally-installed package can run at all', () => {
  ...
  return { ok: r.code === 0 && /claude-mem-lite/.test(r.stdout + r.stderr), detail: `exit=${r.code} ${(r.stdout || r.stderr).slice(0, 200)}` };
});
...
check('install exits 0', () => ({ ok: inst.code === 0, detail: `exit=${inst.code} ${(inst.stdout + inst.stderr).slice(-700)}` }));
check('code deployed into ~/.claude-mem-lite', () =>
  existsSync(join(DATA, 'server.mjs')) && existsSync(join(DATA, 'hook.mjs')) && existsSync(join(DATA, 'cli.mjs')));
...
check('save writes a memory', () => {
  const r = run(CLI, ['save', '--type', 'bugfix', '--lesson', 'Stale ABI bindings must be rebuilt, not reinstalled.', 'Sandbox npm-form smoke memory about sqlite binding repair'], { env: ENV, cwd: PROJECT });
...
check('search finds it back', () => {
  ...
  return { ok: r.code === 0 && /Sandbox npm-form smoke/i.test(r.stdout), detail: `exit=${r.code} ${(r.stdout || r.stderr).slice(0, 300)}` };
});
for (const [label, args] of [['recent', ['recent', '3']], ['stats', ['stats']], ['timeline', ['timeline']], ['activity', ['activity', 'recent']], ['registry', ['registry', 'list']]]) {
  const r = run(CLI, args, { env: ENV, cwd: PROJECT });
  check(`CLI ${label} exits 0`, () => ({ ok: r.code === 0, detail: `exit=${r.code} ${(r.stdout || r.stderr).slice(0, 200)}` }));
}
...
for (const [event, groups] of Object.entries(settings.hooks || {})) {
  for (const g of groups) {
    for (const h of g.hooks || []) {
      ...
      const payload = {
        session_id: SESSION, cwd: PROJECT, hook_event_name: event, source: 'startup',
...
check('every registered hook exits 0', () => {
  ...
  return { ok: bad.length === 0, detail: bad.map((b) => `${b.event}: exit=${b.code} ${b.stderr.slice(0, 200)}`).join('\n') || `${fired.length} hooks fired clean` };
});
...
check('no hook mixes a JSON envelope with raw prose on one stdout', () => {
  const bad = fired.filter((f) => {
    ...
    if (!s) return false;
    try { JSON.parse(s); return false; } catch { /* not one document — look closer */ }
    return s.split('\n').some((l) => l.trim().startsWith('{'));
  });
  return {
    ok: bad.length === 0,
    detail: bad.map((b) => `${b.event}: ${b.stdout.slice(0, 220)}`).join('\n') || `${fired.length} surfaces clean`,
  };
});
check('SessionStart specifically emits exactly one envelope', () => {
  const ss = fired.filter((f) => f.event === 'SessionStart' && f.stdout.trim());
  const bad = ss.filter((f) => { try { JSON.parse(f.stdout.trim()); return false; } catch { return true; } });
  return { ok: bad.length === 0, detail: bad.map((b) => b.stdout.slice(0, 220)).join('\n') || `${ss.length} SessionStart write(s), all single-envelope` };
});
...
const mcp = await mcpSession(process.execPath, [join(DATA, 'server.mjs')], {
  ...
  requests: [
    ...
    { jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'mem_save', arguments: { content: 'MCP-form save from the npm install sandbox', title: 'npm-form MCP smoke', type: 'decision', lesson_learned: 'The managed install runs server.mjs directly, not through launch.mjs.' } } },
    { jsonrpc: '2.0', id: 4, method: 'tools/call', params: { name: 'mem_search', arguments: { query: 'npm-form MCP smoke' } } },
  ],
});
check('MCP initialize + tools/list', () => {
  const a = mcp.responses.find((x) => x.id === 1);
  const b = mcp.responses.find((x) => x.id === 2);
  return { ok: !!a?.result?.serverInfo && (b?.result?.tools || []).length === 9, detail: `${(b?.result?.tools || []).length} tools; ${mcp.stderr.slice(0, 200)}` };
});
check('MCP mem_save then mem_search round-trips', () => {
  ...
  return { ok: !s?.result?.isError && /npm-form MCP smoke/i.test(qt), detail: `${(s?.result?.content?.[0]?.text || '').slice(0, 120)} || ${qt.slice(0, 200)}` };
});
...
check('update did not damage the install', () => {
  const r = run(CLI, ['doctor'], { env: ENV, cwd: PROJECT, timeout: 120_000 });
  return { ok: r.code === 0, detail: `doctor exit=${r.code} ${(r.stdout || '').split('\n').filter((l) => /✗/.test(l)).join(' | ')}` };
});
...
if (existsSync(bind)) {
  ...
  check('control: the CLI\'s OWN tree is still healthy (so a green verdict would be the bug)', () => {
    const r = node(['-e', `const {createRequire}=require('node:module');const D=createRequire(${JSON.stringify(join(globalPkgDir, 'package.json'))})('better-sqlite3');new D(':memory:').close()`], { env: ENV });
    return { ok: r.code === 0, detail: `exit=${r.code}` };
  });
  ...
  check('a hook fire on a broken binding still exits 0 (never spams a stack trace)', () => {
    const r = runHook(`node "${join(DATA, 'scripts', 'hook-launcher.mjs')}" scripts/pre-tool-recall.js`, {
      session_id: SESSION, cwd: PROJECT, hook_event_name: 'PreToolUse', tool_name: 'Edit',
  ...
  check('doctor NAMES the broken binding instead of going quietly green', () => {
    const r = run(CLI, ['doctor'], { env: ENV, cwd: PROJECT, timeout: 120_000 });
    return { ok: r.code === 1 && /binding/i.test(r.stdout), detail: `exit=${r.code} ${(r.stdout || '').split('\n').filter((l) => /✗|binding/i.test(l)).join(' | ').slice(0, 300)}` };
  });
  ...
  check('binding loads again', () => {
    const r = node(['-e', `const {createRequire}=require('node:module');const D=createRequire(${JSON.stringify(join(nmHost, 'package.json'))})('better-sqlite3');new D(':memory:').close()`], { env: ENV });
    return { ok: r.code === 0, detail: `exit=${r.code} ${r.stderr.split('\n')[0]}` };
  });
  check('doctor is green again after the repair', () => {
    const r = run(CLI, ['doctor'], { env: ENV, cwd: PROJECT, timeout: 120_000 });
    return { ok: r.code === 0, detail: `exit=${r.code} ${(r.stdout || '').split('\n').filter((l) => /✗/.test(l)).join(' | ')}` };
  });
}
...
check('uninstall exits 0', () => ({ ok: un.code === 0, detail: `exit=${un.code} ${(un.stdout + un.stderr).slice(-400)}` }));
...
check('uninstall --purge exits 0', () => ({ ok: pur.code === 0, detail: `exit=${pur.code} ${(pur.stdout + pur.stderr).slice(-400)}` }));
check('--purge removes the data dir', () => ({ ok: !existsSync(join(DATA, 'claude-mem-lite.db')), detail: existsSync(DATA) ? readdirSync(DATA).join(',') : '(dir gone)' }));
