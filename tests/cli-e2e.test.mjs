...
describe('registry bare value-less flag (Round2-P1)', () => {
  ...
  // Same bare-flag class (#8470) reached more string-valued flags that were never
  // guarded: a trailing `--fields` / `--files` / `--body` parsed to boolean `true` and
  // crashed at `.split()` or the SQLite bind with a raw stacktrace. Now all routed
  // through rejectBareStringFlags (cli/common.mjs).
  it.each([
    [['get', '1', '--fields'], '--fields'],
    [['save', 'some text', '--files'], '--files'],
    [['save', 'some text', '--title'], '--title'],
    [['defer', 'add', 'a title', '--detail'], '--detail'],
    [['activity', 'save', '--type', 'bugfix', 'a title', '--body'], '--body'],
    [['registry', 'search', '--query'], '--query'],
    [['timeline', '--query'], '--query'],
  ])('rejects bare %s cleanly (no raw stacktrace)', (args) => {
  ...
  // Bare `--project` crashed many read commands at the root helper resolveProject(true)
  // (`true.includes("--")` TypeError). Hardened to treat non-string as "no filter" — these
  // must now run to completion (any non-zero is fine for empty data, but NO raw stacktrace).
  it.each([
    ['search', ['search', 'anything', '--project']],
    ['recent', ['recent', '--project']],
    ['timeline', ['timeline', '--project']],
    ['stats', ['stats', '--project']],
    ['export', ['export', '--project']],
  ])('bare --project on %s does not crash with a raw stacktrace', (_name, args) => {
    const { stderr, stdout } = runCli(args);
    expect(stderr).not.toContain('TypeError');
    expect(stderr).not.toContain('project-utils.mjs');
    expect(stdout + stderr).not.toContain('.includes is not a function');
  });

  // activity save --importance accepted trailing-garbage tokens ("3xyz" → 3) and wrote
