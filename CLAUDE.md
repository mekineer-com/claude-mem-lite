...
- **Version**: 3.71.0
- **Package manager**: npm
- **Test**: `npx vitest run` (273 test files / 4633 tests, vitest) · **Sandbox install harness** (not in `vitest run`; real `npm i -g` + real MCP stdio, minutes + network): `node tests/sandbox/phaseA-plugin.mjs` / `phaseB-npm.mjs` / `phaseC-update.mjs` — see `tests/sandbox/README.md`
- **Lint**: `npx eslint .`
